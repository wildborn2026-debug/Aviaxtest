...
class Call(PyTgCalls):
    ...
    async def _play(self, client, chat_id, stream):
        await client.play(
            chat_id,
            stream,
    ...
    async def speedup_stream(self, chat_id: int, file_path, speed, playing):
        ...
        stream = self._build_stream(out, video=video_mode, ffmpeg=f"-ss {played} -to {duration}")
    ...
    async def seek_stream(self, chat_id, file_path, to_seek, duration, mode):
        assistant = await group_assistant(self, chat_id)
        stream = self._build_stream(file_path, video=mode == "video", ffmpeg=f"-ss {to_seek} -to {duration}")
        await self._play(assistant, chat_id, stream)

    async def stream_call(self, link):
        assistant = await group_assistant(self, config.LOG_GROUP_ID)
        stream = self._build_stream(link, video=True)
        await self._play(assistant, config.LOG_GROUP_ID, stream)
    ...
    async def decorators(self):
        for string, client in [
            ...
            @client.on_update()
            async def _update_handler(_, update: Update, _client=client):
